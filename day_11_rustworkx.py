...
#add node for output statically, since its not in the input set
node_lut["out"] = G.add_node("out")

#add all nodes to graph and lut
for node_description in used_input.split("\n"):
    node = node_description.split(":")[0]
    node_lut[node] = G.add_node(node)


#add connections between the nodes
for node_description in used_input.split("\n"):
    node = node_description.split(":")[0]
    for destination in node_description.split(":")[1].split(" ")[1:]:
        G.add_edge(node_lut[node], node_lut[destination], None)


num_of_ways = 0

###Part 1
for _ in rx.all_simple_paths(G, node_lut["you"], node_lut["out"]):
    num_of_ways += 1

print("Part 1, number of ways between \"you\" and \"out\": ", num_of_ways)
